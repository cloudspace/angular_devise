      ...
      function ($q, $http, $rootScope) {
        ...
        var service = {
            ...
            update: function (creds) {
              creds = creds || {};
              return $http(httpConfig('update', { user: creds })).then(service.parse).then(save).then(broadcast('update-successfully'));
            },
            sendResetPasswordInstructions: function (creds) {
              creds = creds || {};
              return $http(httpConfig('sendResetPasswordInstructions', { user: creds })).then(service.parse).then(broadcast('send-reset-password-instructions-successfully'));
            },
            resetPassword: function (creds) {
              creds = creds || {};
              return $http(httpConfig('resetPassword', { user: creds })).then(service.parse).then(save).then(broadcast('reset-password-successfully'));
            },
